...
function Shows({ openShowsModal, setOpenShowsModal, theatre }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "date",
      render: (text, record) => {
        return moment(text).format("ddd, MMM Do YYYY");
      },
    },
    {
      title: "Time",
        ...
        return (
          <div className="flex gap-1 items-center">
            {record.bookedSeats.length === 0 && (
              <i
                className="ri-delete-bin-fill text-primary cursor-pointer"
                onClick={() => {
  ...
  return (
    <Modal
      ...
      footer={null}
    >
      <h1 className="text-primary text-md mb-1">Theatre : {theatre.name}</h1>
      <hr />
